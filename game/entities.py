...
class Jefe:
    ...
    def mover(self):
        ...
        if self.x < 50 or self.x > ANCHO - 50 - self.w:
            self.direccion *= -1
...
class EnemigoErratico(Enemigo): # SMILE
...
class EnemigoFantasma(Enemigo): # SMOKE
    def __init__(self, x, y): super().__init__(x, y, "SMOKE"); self.velocidad=1.2
    def puede_moverse(self, nx, ny, mapa):
        if 0 <= nx < COLS and 0 <= ny < FILAS:
            return mapa[ny][nx] in [V, B] # Atraviesa basura
        return False
    def mover(self, mapa, jugadores):
        if time.time() > self.timer_mov:
...
class EnemigoCazador(Enemigo): # SLIME
